...
void parse( web_t* web_struct ) {
	...
	int index = 0;
	char *href = "href=\"";
	char *ptr = web_struct->webpage, *ptr2;

	while ((ptr = strstr(ptr, href)) != NULL) {
        if ((ptr2 = strchr(ptr+strlen(href), '"')) != NULL) {
			*ptr2 = '\0';
			if (strstr(ptr+strlen(href), "trump") != NULL) {
				if (web_struct->links == NULL) {
					web_struct->links = (char **)malloc(sizeof(char *) * 256);
					web_struct->link_cnt = 0;
				}				
				web_struct->links[web_struct->link_cnt++] = strdup(ptr);
			}
			*ptr2 = '"';
			ptr = ptr2;
		}
